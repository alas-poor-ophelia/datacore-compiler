```datacorejsx
const { CompilerUI } = await dc.require(dc.headerLink(dc.resolvePath("Datacore Script Compiler"), "CompilerView"));
...
function rewriteCssReferences(fileContent, cssReferences, compiledNoteName) {
  ...
  console.log(`[CSS Rewrite] Processing ${cssReferences.length} CSS references`);
  ...
  const replacements = cssReferences
    ...
    .map(ref => {
      ...
      const replacement = `const ${ref.varName} = await dc.require(dc.headerLink(dc.resolvePath("${compiledNoteName}"), "${moduleName}"))`;
      
      console.log(`[CSS Rewrite] Preparing replacement:`);
      console.log(`  Variable: ${ref.varName}`);
      console.log(`  Module: ${moduleName}`);
      console.log(`  Indices: [${ref.startIndex}, ${ref.endIndex}]`);
      console.log(`  Old length: ${ref.endIndex - ref.startIndex}`);
      console.log(`  New length: ${replacement.length}`);
      console.log(`  Original snippet: ${fileContent.substring(ref.startIndex, Math.min(ref.startIndex + 80, ref.endIndex)).replace(/\n/g, '\\n')}...`);
  ...
  console.log(`[CSS Rewrite] ${replacements.length} replacements will be applied`);
  
  // Check for overlaps
  for (let i = 0; i < replacements.length - 1; i++) {
    for (let j = i + 1; j < replacements.length; j++) {
      if (replacements[i].end > replacements[j].start) {
        console.error(`[CSS Rewrite] WARNING: Overlapping replacements detected!`);
        console.error(`  Replacement ${i}: [${replacements[i].start}, ${replacements[i].end}]`);
        console.error(`  Replacement ${j}: [${replacements[j].start}, ${replacements[j].end}]`);
      }
    }
  }
