...
<!-- Compiled: 2025-11-29T03:12:26.239Z -->
<!-- Files: 12 -->
<!-- Version: 0.5.1 -->
...
const { normalizePath } = await dc.require(dc.headerLink(dc.resolvePath("datacore-compiler"), "utils"));
const { scanDirectory } = await dc.require(dc.headerLink(dc.resolvePath("datacore-compiler"), "fileDiscovery"));
const { extractDependencies } = await dc.require(dc.headerLink(dc.resolvePath("datacore-compiler"), "dependencyAnalyzer"));
const { buildDependencyOrder } = await dc.require(dc.headerLink(dc.resolvePath("datacore-compiler"), "dependencyGraph"));
const { rewriteImports, rewriteCssReferences } = await dc.require(dc.headerLink(dc.resolvePath("datacore-compiler"), "importRewriter"));
const { generateBundle } = await dc.require(dc.headerLink(dc.resolvePath("datacore-compiler"), "bundleGenerator"));
const { writeToVault } = await dc.require(dc.headerLink(dc.resolvePath("datacore-compiler"), "fileWriter"));
const { detectCssReferences, getCssModuleName } = await dc.require(dc.headerLink(dc.resolvePath("datacore-compiler"), "cssReferenceAnalyzer"));
const { minify, minifyWithObfuscation } = await dc.require(dc.headerLink(dc.resolvePath("datacore-compiler"), "minifier"));
